...
function Login() {
  ...
  useEffect(() => {
    if (localStorage.getItem("user-info"))
      if (
        !Object.hasOwn(JSON.parse(localStorage.getItem("user-info")), "error")
      ) {
  ...
  return (
    <>
      <div className="bg-gray-100 min-h-screen flex items-center justify-center ">
        <div
          className="bg-blue-200 flex rounded-2xl  max-w-3xl w-6/12 p-5 "
          style={{
            boxShadow:
        ...
        >
          <div className="sm:w-1/2 px-8 flex flex-col justify-center">
            ...
            <div className="flex justify-between">
              <span
                className="text-sm  mt-1 cursor-pointer bg-slate-200 p-1 px-2 rounded shadow-md"
                onClick={() => navigate("/Ecommerce-project/SignUp")}
              >
                <u>Sign Up</u>{" "}
              </span>
              <span className="text-sm  mt-1 cursor-pointer">
                <u>Forget password?</u>{" "}
              </span>
            </div>
          </div>
